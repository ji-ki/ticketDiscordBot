...
async def view_tickets(ctx):
    try:
        async with bot.pg_pool.acquire() as connection:
            ...
            records = await connection.fetch('SELECT user_id, username, message FROM tickets')
            if records:
                embed = discord.Embed(title="Список тикетов:\n", color=0x00ff00)
                for record in records:
                    embed.add_field(name=f"ID: {record['user_id']}", value=f"Пользователь: {record['username']}\nСообщение: {record['message']}", inline=False)
                await ctx.send(embed=embed)
            else:
...
async def close(ctx, ticket_id: int):
    try:
        async with bot.pg_pool.acquire() as connection:
            ...
            ticket = await connection.fetchrow('SELECT id FROM tickets WHERE id = $1', ticket_id)
            if ticket:
                # Удаляем тикет
                await connection.execute('DELETE FROM tickets WHERE id = $1', ticket_id)
